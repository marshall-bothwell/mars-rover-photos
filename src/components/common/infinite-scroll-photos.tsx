...
export default function InfiniteScrollPhotos({ roverPhotos, pageSize }: InfiniteScrollPhotosProps) {
    ...
    return (
        <div>
            <div className="grid md:grid-cols-2 xl:grid-cols-3 w-[95vw]">{roverPhotos?.slice(0, page * pageSize)}</div>
            {page >= maxPages || !maxPages ? null : (
                <div className="h-96 flex flex-col justify-end items-center">
                    <div className="flex grow mb-auto text-sm">Keep scrolling to load more photos...</div>
